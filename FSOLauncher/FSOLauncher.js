...
class FSOLauncher extends Events {
        ...
        return new Promise(async (resolve, reject) => {
          if (!options.override) {
            ...
            /*
                        ...
                        to SOEzilii.exe, because they will surely nuke their game if
  ...
  play(useVolcanic) {
    ...
    fs.stat(this.isInstalled.FSO + "\\SOEzilii.exe", (err, stat) => {
      if (err === null) {
        this.launchGame();
      } else if (err.code === "ENOENT") {
        fs.stat(this.isInstalled.FSO + "\\SOEzilii.exe.old", (err, stat) => {
          if (err === null) {
            fs.rename(
              this.isInstalled.FSO + "\\SOEzilii.exe.old",
              this.isInstalled.FSO + "\\SOEzilii.exe",
              err => {
  ...
  launchGame(useVolcanic) {
    let file = useVolcanic ? "Volcanic.exe" : "SOEzilii.exe";
  ...
  /**
   * Creates a shortcut for SOEzilii.exe.
   *
   * @deprecated
